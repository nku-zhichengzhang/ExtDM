...
def window_partition(x, window_size):
    ...
    windows = rearrange(x, 'B C t p1 h p2 w p3-> (B t h w) (p1 p2 p3) C')
    return windows
...
def window_reverse(windows, window_size, B, T, H, W):
    ...
    Returns:
        x: (B, D, H, W, C)
    """
    x = rearrange(windows, '(B t h w) (p1 p2 p3) C -> B C (t p1) (h p2) (w p3)', B=B, t=T//window_size[0], h=H//window_size[1], w=W//window_size[2], p1=window_size[0], p2=window_size[1], p3=window_size[2])
    return x
...
class STWAttentionLayer(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.norm = norm_layer(dim)
        self.attn = WindowAttention3D(dim, num_heads=heads, rotary_emb=rotary_emb)
...
class Unet3D(nn.Module):
    def __init__(
        ...
        # TODO:
        spatial_attn = EinopsToAndFrom('b c f h w', 'b f (h w) c', AttentionLayer(mid_dim, heads=attn_heads, dim_head=mid_dim//attn_heads))
    ...
    def forward(self, x, time, cond_frames, cond_fea=None, cond=None, null_cond_prob=0., none_cond_mask=None):
        ...
        f = self.final_conv(x)[:,:,tc:]
        o = self.occlusion_map(x)[:,:,tc:]
        return torch.cat((f, o), dim=1)