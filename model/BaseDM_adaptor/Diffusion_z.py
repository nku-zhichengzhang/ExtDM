...
class GaussianDiffusion(nn.Module):
    ...
    def p_mean_variance(self, x_cond, x, cond_fea, t, clip_denoised: bool, cond=None, cond_scale=7.5):
        ...
        epsilon_noise, _ = self.denoise_fn(x, t, cond_frames=x_cond, cond=cond, cond_fea=cond_fea, cond_scale=cond_scale)
    ...
    def sample(self, x_cond, cond_fea, cond=None, cond_scale=1., batch_size=16):
        ...
        sample_fn = self.p_sample_loop if not self.is_ddim_sampling else self.dpm_solver_sample
        
        return sample_fn(x_cond, (batch_size, channels, num_frames, x_cond.shape[3], x_cond.shape[4]), cond_fea=cond_fea, cond=cond, cond_scale=cond_scale)

    @torch.no_grad()
    def dpm_solver_sample(self, x_cond, shape, cond_fea, cond=None, cond_scale=1., clip_denoised=True):
        
        batch, tc, tp, device = shape[0], x_cond.shape[2], shape[2], self.betas.device
        img = torch.randn(shape, device=device)
        ns = NoiseScheduleVP('discrete', alphas_cumprod=self.alphas_cumprod)
        
        model_fn = model_wrapper(
            lambda x, t: self.denoise_fn(x, t, cond_frames=x_cond, cond=cond, cond_fea=cond_fea),
            ns,
            model_type="noise",
            guidance_type="uncond",
        )
        dpm_solver = DPM_Solver(model_fn, ns, predict_x0=True, thresholding=True)
        x = dpm_solver.sample(img, steps=self.sampling_timesteps, skip_type="logSNR", method="multistep", order=2)
    ...
    def p_losses(self, x_start_cond, x_start_pred, cond_fea, t, cond=None, noise=None, clip_denoised=True, **kwargs):
        ...
        n,c,tc,h,w = x_start_cond.size()
        x_noisy = self.q_sample(x_start=x_start_pred, t=t, noise=noise)
        pred_noise, pred_cond = self.denoise_fn.forward(x_noisy, t, cond_fea=cond_fea, cond_frames=x_start_cond, cond=cond, null_cond_prob=self.null_cond_prob, none_cond_mask=None, **kwargs)
        zero_noise = torch.zeros((n,c,tc-1,h,w), device=pred_noise.device)
        
        print(noise.shape)
        if self.loss_type == 'l1':
            loss = F.l1_loss(torch.cat([zero_noise, noise], dim=2), torch.cat([pred_cond, pred_noise]))
        elif self.loss_type == 'l2':
            loss = F.mse_loss(torch.cat([zero_noise, noise], dim=2), torch.cat(pred_cond, pred_noise), dim=2)
        else:
