...
def get_3d_sincos_pos_embed(embed_dim, grid_size, t_size, cls_token=False):
    ...
    pos_embed = rearrange(pos_embed, 't (h w) d->1 d t h w', h=grid_size, w=grid_size)
...
class Unet3D(nn.Module):
    def __init__(
        ...
        # initial convond_num+pred_num-1
        ...
        pos_emb = get_3d_sincos_pos_embed(pos_embed_dim*2, framesize, T)
        ...
        self.noise_conv = nn.Conv3d(3, pos_embed_dim, kernel_size=(1, init_kernel_size, init_kernel_size), padding=(0, init_padding, init_padding))
                     torch.from_numpy(pos_emb).float()
                    )
