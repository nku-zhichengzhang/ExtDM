...
class adaptor(nn.Module):
    def __init__(self, dim, num_layer=2):
        ...
        for _ in range(num_layer):
            self.extrapolators.append(nn.ModuleList([
                Residual(PreNorm(dim, nn.Conv3d(dim,dim,1, bias=False))),
    ...
    def forward(self, xm):
        ...
        x = xm
        for estimator, m_estimator, v_estimator in self.extrapolators:
            r = x
...
class Unet3D(nn.Module):
    ...
    def forward(self, x, time, cond_frames, cond_fea=None, cond=None, null_cond_prob=0., none_cond_mask=None):
        ...
        for block1, STW_attn1 ,block2, STW_attn2, downsample in self.downs:
            x = block1(x, t)
            x = STW_attn1(x)
            x = block2(x, t)
            x = STW_attn2(x)
        ...
        for block1, STW_attn1 ,block2, STW_attn2, upsample in self.ups:
