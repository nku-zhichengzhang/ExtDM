...
class Unet3D(nn.Module):
    ...
    def forward(
        ...
        return torch.cat((self.final_conv(x), self.occlusion_map(x)), dim=1)[:,:,:-tc]
...
class GaussianDiffusion(nn.Module):
    ...
    def ddim_sample(self, x_cond, fea, shape, cond=None, cond_scale=1., clip_denoised=True):
        ...
        for time, time_next in tqdm(time_pairs, desc='sampling loop time step'):
            ...
            pred_noise = self.denoise_fn.forward_with_cond_scale(
                ...
                time_cond,
                cond_frames=torch.cat([x_cond, fea.repeat(1, 1, x_cond.size(2), 1, 1)], dim=1),
                cond=cond,
                cond_scale=cond_scale)
    ...
    def p_losses(self, x_start_cond, x_start_pred, t, fea, cond=None, noise=None, clip_denoised=True, **kwargs):
        ...
        #     cond = cond.to(device)
        pred_noise = self.denoise_fn.forward(torch.cat([x_noisy, fea.repeat(1, 1, x_noisy.size(2), 1, 1)], dim=1), t, cond_frames=torch.cat([x_start_cond, fea.repeat(1, 1, x_start_cond.size(2), 1, 1)], dim=1), cond=cond, null_cond_prob=self.null_cond_prob, none_cond_mask=None, **kwargs)
