...
class GaussianDiffusion(nn.Module):
    ...
    def ddim_sample(self, x_cond, fea, shape, cond=None, cond_scale=1., clip_denoised=True):
        ...
        img = torch.randn(shape, device=device)
        # img[:,:,:x_cond.size(2)] = x_cond
        fea = fea.unsqueeze(dim=2).repeat(1, 1, img.size(2)+x_cond.size(2), 1, 1)
        ...
        for time, time_next in tqdm(time_pairs, desc='sampling loop time step'):
            ...
            pred_noise = self.denoise_fn.forward_with_cond_scale(
                ...
                cond=cond,
                cond_scale=cond_scale)
            pred_noise = pred_noise[:,:,x_cond.size(2):]
    ...
    def p_losses(self, x_start_cond, x_start_pred, t, fea, cond=None, noise=None, clip_denoised=True, **kwargs):
        ...
        x_noisy = torch.cat([x_start_cond, x_noisy], dim=2)

        # if is_list_str(cond):
        #     none_cond_mask = [ii == "None" for ii in cond]
        #     cond = bert_embed(tokenize(cond), return_cls_repr=self.text_use_bert_cls)
        #     cond = cond.to(device)
        pred_noise = self.denoise_fn.forward(torch.cat([x_noisy, fea], dim=1), t, cond=cond, null_cond_prob=self.null_cond_prob, none_cond_mask=None, **kwargs)
        pred_noise = pred_noise[:,:,x_start_cond.size(2):]
        ...
        pred_x0 = self.predict_start_from_noise(x_noisy[:,:,x_start_cond.size(2):], t, pred_noise)
    ...
    def forward(self, x_cond, x_pred, fea, *args, **kwargs):
        ...
        fea = fea.unsqueeze(dim=2).repeat(1, 1, x_cond.size(2)+x_pred.size(2), 1, 1)
        # x = normalize_img(x)
