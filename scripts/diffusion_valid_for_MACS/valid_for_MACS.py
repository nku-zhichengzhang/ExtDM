...
# from model.BaseDM_adaptor_for_MACs.VideoFlowDiffusion_multi1248 import FlowDiffusion
from model.BaseDM_adaptor_for_MACs.VideoFlowDiffusion_multi_w_ref import FlowDiffusion
# from model.BaseDM_adaptor_for_MACs.VideoFlowDiffusion_multi_w_ref_u22 import FlowDiffusion
...
if __name__ == "__main__":
    ...
    parser.add_argument("--fp16", 
                        default=False)
    parser.add_argument("--train_pred_frames", 
                        type=int, 
                        default=1)
    ...
    config['dataset_params']['train_params']['pred_frames'] = args.train_pred_frames
    ...
    for i_iter, batch in enumerate(valid_dataloader):
        ...
        for i_autoreg in range(NUM_AUTOREG):
            ...
            # 带有matmul矩阵乘法计算
            # 1     565 826 368 808.0 =   565 GMacs
            # 2   1 084 985 832 744.0 = 1 084 GMacs
            # 10  5 238 261 544 232.0 = 7 179 GMacs
            # 每个 step 单独 519 159 463 936 = 519 GMacs
            from thop import profile
            macs, _ = profile(model, inputs=(1.0, i_real_vids.cuda()))
            print(macs)
